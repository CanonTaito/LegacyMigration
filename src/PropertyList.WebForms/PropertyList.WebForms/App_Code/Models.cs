...
namespace PropertyList.WebForms
{
    ...
    {
        private static readonly Lazy<List<Property>> _properties = 
            new Lazy<List<Property>>(() => InitializePropertyData());
        ...
        {
            return _properties.Value.ToList();
        }

        public Property GetById(int id)
        {
            var property = _properties.Value.FirstOrDefault(p => p.Id == id);
            if (property == null)
        ...
        {
            var results = _properties.Value.AsQueryable();
            ...
            if (!string.IsNullOrEmpty(propertyType) && propertyType != "")
            {
        ...
        public void UpdateProperty(Property property)
        {
            ...
            var existingProperty = _properties.Value.FirstOrDefault(p => p.Id == property.Id);
            if (existingProperty == null)
                throw new KeyNotFoundException($"Property with ID {property.Id} not found for update");
            
            // Update existing property
            var index = _properties.Value.IndexOf(existingProperty);
            _properties.Value[index] = property;
        }
        ...
        public void DeleteProperty(int id)
        {
            ...
            _properties.Value.Remove(property);
        }
        
        private static List<Property> InitializePropertyData()
        {
